...
def read_csv():
    data = pd.read_csv('C:\\Users\\anura\\Documents\\Python Scripts\\Assignment1\\top100textrankdesc.csv', skiprows=1, names=["keyWords","id","freq"])
...
def boa_word_count(list_keywords, row):
    ...
    href_list = []
    dict={}
    while countPage >= 1 and countPage < 20:
        link_count=1
...
def write_excel(list_keywords):
    workbook = xlsxwriter.Workbook('textrankScrape.xlsx', {'strings_to_urls': False})
...
def csv_from_excel():
    wb = xlrd.open_workbook('textrankScrape.xlsx')
    sh = wb.sheet_by_name('Sheet1')
    your_csv_file = open('textrank_csv_file.csv', 'w')
